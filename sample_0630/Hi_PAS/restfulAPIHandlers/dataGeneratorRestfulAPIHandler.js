var dbManager = require('../utility/dbManager/commonDBManager.js');
var async = require('async');

var machineRealTimeDataJson = require("../models/pdas/machineRealTimeData.json");
var machineCycleDataJson = require("../models/pdas/machineCycleData.json");
var machineErrorDataJson = require("../models/pdas/machineErrorData.json");
...
module.exports.GenerateDummyData = function (req, res) {  
    ...
    async.series([
        function (callback) {
            ...
            dbManager.BulkInsert(arrMCRealTimeData, callback);
        },
        function (callback) {
            GenerateMachineCycleData(req, arrMachineCycleData, arrErrorData);
            dbManager.BulkInsert(arrMachineCycleData, callback);
        },
        function (callback) {
            if (arrErrorData.length > 0)
                dbManager.BulkInsert(arrErrorData, callback);
            else callback();
        }
    ], function (err, result) {
        if (err) {
...
var GenerateMachineRealTimeData = function (req, arrMCRealTimeData) {
    ...
    for (var tmpDate = new Date(periodFrom); tmpDate <= periodTo; tmpDate.setDate(tmpDate.getDate() + 1)) {
        ...
        for (var cnt = 1; cnt < (baseWorkHour * 3600); cnt += 2) {  // 한번에 2건(초)의 전류 Data 가 들어온다 가정 
            ...
            machineRealTimeData.properties.TimeStamp = timeStamp.toJSON();
            machineRealTimeData.properties.SC_No = 1;
            var drivingMotor = [];
            var hoistingMotor = [];
            var forkMotor = [];
            for (var n = 0; n < 2; n++) {
            ...
            machineRealTimeData.properties.DrivingMotor = drivingMotor;
            machineRealTimeData.properties.HoistingMotor = hoistingMotor;
            machineRealTimeData.properties.ForkMotor = forkMotor;
...
var GenerateMachineCycleData = function (req, arrMachineCycleData, arrErrorData) {
    ...
    for (var tmpDate = new Date(periodFrom); tmpDate <= periodTo; tmpDate.setDate(tmpDate.getDate() + 1)) {
        ...
        while (dailyWorkLoad > 0) {
            ...
            machineCycleData.properties.JobID = targetSC.jobID++;
            machineCycleData.properties.JobType = targetSC.JobType == 0 ? 1 : 0;
            machineCycleData.properties.SC_No = targetSC.scNo;
            ...
            machineCycleData.properties.DrivingInfo = arrDrivingInfo;
            machineCycleData.properties.UpperDrivingInfo = arrUpperDrivingInfo;
            machineCycleData.properties.HoistingInfo = arrHoistingInfo;
            machineCycleData.properties.ForkInfo = arrForkInfo;
            machineCycleData.properties.TotalStartTime = startTime;
            machineCycleData.properties.TotalEndTime = endTime;

            targetSC.LaserDistanceMeterTotalUsedTime += endTime - startTime;
            targetSC.OpticalRepeaterTotalUsedTime += endTime - startTime;
            machineCycleData.properties.LaserDistanceMeterTotalUsedTime = targetSC.LaserDistanceMeterTotalUsedTime; //ms
            machineCycleData.properties.OpticalRepeaterTotalUsedTime = targetSC.OpticalRepeaterTotalUsedTime; //ms
            machineCycleData.properties.Weight = 100;
            machineCycleData.properties.InventoryCount = 200;
...
var GenerateErrorData = function (cycleInfo) {
    ...
    errorData.properties.SCNo = cycleInfo.scNo;
    errorData.properties.CurrDate = cycleInfo.currTime;
    errorData.properties.JobNo = cycleInfo.jobID;
    errorData.properties.ErrCode = Math.floor(Math.random() * 1000) + 1;
    errorData.properties.ErrMsg = "Dummy error message";
    return errorData;
}